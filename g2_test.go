...
func TestG2Affine_SerializeBytes(t *testing.T) {
	r := NewXORShift(1)

	x, _ := bls.RandFQ2(r)
	y, _ := bls.RandFQ2(r)

	g2 := bls.NewG2Affine(x, y)

	bytes := g2.SerializeBytes()

	g2Other := new(bls.G2Affine)
	err := g2Other.SetRawBytes(bytes)
	if err != nil {
		t.Fatal(err)
	}

	if !g2.Equals(g2Other) {
